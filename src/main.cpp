...
//(this message has a DLC of 1, with bit 0 being the ignition flag and bit 1
//being the reverse flag)
...
int max_rpm = (MAX_VELOCITY_MPH * 0.44704) / WHEEL_RADIUS_M *
              9.5493;  // 1 mph = 0.44704 m/s and 1 rad/s = 9.5493 rpm
...
int main(void) {
    ...
    while (1) {
        // Check if ignition is toggled and update bit 0 of drive_message_array
        // before sending a drive message
        if (ignition_state != prev_ignition_state) {
            ...
            if (ignition_state) {
                ...
                send_drive_message();
                read_pedals();
                while ((throttle_percentage > 0 || nitrous_state == false) &&
                       ignition_state) {
                    read_pedals();
                }
            }
        ...
        // Check if the reverse switch is toggled and update bit 1 of
        // drive_message_array before sending a drive message
        ...
        // Check if the solar switch is toggled and update bit 2 of
        // drive_message_array before sending a drive message
        ...
        if (ignition_state /*ADD: AND BRAKE ISN'T PRESSED*/) {
            if (heartbeat_flag) {
                ...
                send_motor_drive_command(motor_rpm_demand,
...
void read_pedals() {
    ...
    throttle_percentage = (throttle_percentage - THROTTLE_POT_MIN) /
                          (THROTTLE_POT_MAX - THROTTLE_POT_MIN);
    ...
    if (reverse_state == 1) motor_rpm_demand = -motor_rpm_demand;
}

void ignition_pressed() { ignition_state = !ignition_state; }

void nitrous_pressed() { nitrous_state = !nitrous_state; }
...
void send_motor_drive_command(int rpm, float current) {
    ...
    CANMessage motor_drive_command_l(DRIVER_CONTROLS_BASE_ADDR_L + 0X01,
                                     motor_drive_array_l, 8);
    CANMessage motor_drive_command_r(DRIVER_CONTROLS_BASE_ADDR_R + 0X01,
                                     motor_drive_array_r, 8);
